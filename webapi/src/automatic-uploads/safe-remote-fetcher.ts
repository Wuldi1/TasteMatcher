...
/** Fetches allowlisted Phillips resources with redirect and byte controls. */
@Injectable()
      ...
      for (
        ...
        try {
          ...
          if (controller.signal.aborted) {
            throw new RemoteFetchError(
              "timeout",
              "The remote Phillips request timed out.",
              true,
            );
          }
          throw new RemoteFetchError(
            ...
            error instanceof Error
              ? `The remote Phillips request failed: ${error.message}`
              : "The remote Phillips request failed.",
            true,
          );
        }
        ...
        if (this.isRedirect(response.status)) {
          ...
          if (!location) {
            throw new RemoteFetchError(
              "redirect_blocked",
              "Phillips returned a redirect without a location.",
            );
          }
          if (redirectCount === this.options.maxRedirects) {
            throw new RemoteFetchError(
              "too_many_redirects",
              "Phillips returned too many redirects.",
            );
          }
          try {
            ...
            throw new RemoteFetchError(
              "redirect_blocked",
              "Phillips redirected to a URL that is not allowed.",
            );
          }
          continue;
        }
        ...
        if (!response.ok) {
          throw new RemoteFetchError(
            "http_error",
            `Phillips returned HTTP ${response.status}.`,
  ...
  private validateUrl(value: string, kind: RemoteResourceKind): URL {
    ...
    if (
      ...
      throw new BadRequestException(
        kind === "html"
          ? "Only HTTPS Phillips auction URLs are supported."
          : "The Phillips image URL is not allowed.",
      );
    }
    return url;
  }
  ...
  private assertContentType(
    ...
    if (!allowed.has(contentType)) {
      throw new RemoteFetchError(
        "unsupported_content_type",
        kind === "html"
          ? "Phillips did not return an HTML document."
          : "Phillips did not return a supported JPEG or PNG image.",
      );
    }
  }
