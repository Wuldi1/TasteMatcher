...
export class UsersService {
  ...
  async findAllInDomain(
    ...
    try {
      let query = {
        query:
          "SELECT * FROM c WHERE c.domainId = @domainId AND c.type = 'user' ORDER BY c.createdAt DESC",
      ...
      if (askingUser?.role === "dealer") {
        query = {
          query:
            "SELECT * FROM c WHERE c.domainId = @domainId AND c.type = 'user' AND c.invitedBy = @invitedBy AND c.role = @role ORDER BY c.createdAt DESC",
          parameters: [
      ...
      this.logger.log(
        `Fetched ${resources.length} users for domain ${domainId}`,
      );
      return resources;
    } catch (error) {
  ...
  async update(
    ...
    try {
      ...
      this.logger.error(
        `Failed to update user ${userId}, on domain ${domainId}`,
        error,
      );
      throw error;
    }
  ...
  async remove(
    ...
    try {
      ...
      this.logger.log(
        `Deleted ${preferences.length} preferences for user ${userId}`,
      );
  ...
  async inviteUser(
    ...
    try {
      ...
      const crossDomainQuery = {
        query:
          "SELECT TOP 1 c.id, c.domainId FROM c WHERE c.type = 'user' AND c.email = @email",
      ...
      if (matchInAnotherDomain) {
        throw new BadRequestException(
          `User with email ${inviteDto.email} already exists in another domain`,
        );
      }
      ...
      const existingQuery = {
        query:
          "SELECT * FROM c WHERE c.domainId = @domainId AND c.email = @email AND c.type = 'user'",
        parameters: [
          { name: "@domainId", value: domainId },
      ...
      if (existing.length > 0) {
        ...
        throw new BadRequestException(
          `User with email ${inviteDto.email} already exists in this domain`,
        );
      }
      ...
      this.logger.log(
        `Created new user ${newUser.id} in domain ${domainId} with status pending_verification`,
      );
  ...
  async updateQuestionnaire(
    ...
    try {
      ...
      this.logger.error(
        `Failed to update questionnaire for user ${userId}`,
        error,
      );
      throw error;
    }
  ...
  async completeOnboarding(userId: string, domainId: string): Promise<User> {
    try {
      ...
      const { resource } = await container.item(userId, domainId).patch([
        ...
        {
          op: "set",
          path: "/personalQuestionnaire/completedAt",
          value: Date.now(),
        },
        { op: "replace", path: "/updatedAt", value: Date.now() },
      ]);
  ...
  async vectorizePreferenceImage(
    ...
    try {
      ...
      this.logger.log(
        `Generated embedding vector with ${embedding.vector.length} dimensions`,
      );
      ...
      if (section === "shared_gallery") {
        ...
        const currentCollection =
          user.personalQuestionnaire?.personalCollection || {};
        const collectionImages = [
          ...(currentCollection.imageUrls ?? []),
          blobUrl,
        ];
        ...
        const currentAdmiration =
          user.personalQuestionnaire?.aestheticAdmiration || {};
        const admirationImages = [
          ...(currentAdmiration.imageUrls ?? []),
          blobUrl,
        ];
        updatedUser.personalQuestionnaire = {
          ...updatedUser.personalQuestionnaire,
      ...
      this.logger.log(
        `Processed preference image ${resultingCount} for user ${userId}`,
      );
      ...
      this.logger.error(
        `Failed to vectorize preference image for user ${userId}`,
        error,
      );
      throw error;
    }
  ...
  async finalizePreferenceVectors(
    ...
    try {
      ...
      this.logger.log(
        `Finalizing ${tempVectors.length} preference vectors for user ${userId}`,
      );
      ...
      this.logger.error(
        `Failed to finalize preference vectors for user ${userId}`,
        error,
      );
      throw error;
    }
