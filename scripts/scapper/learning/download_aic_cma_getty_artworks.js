...
async function downloadImageWithRetry(url, outputPath, retries = 3, referer = '', xsrfToken = '') {
...
const AIC_FIELDS = [
  ...
  'image_id','style_title','place_of_origin','thumbnail.alt_text'
].join(',');
...
function mapAICToStandard(art) {
  return {
    title: art.title || '',
    description: art.thumbnail?.alt_text || `${art.artist_display || ''} ${art.medium_display || ''}`.trim(),
    ...
    tags: [art.style_title, art.medium_display, art.classification_title].filter(Boolean)
  };
}

function getAICImageURL(imageId) {
  return imageId ? `https://www.artic.edu/iiif/2/${imageId}/full/843,/0/default.jpg` : null;
}
...
async function processArtwork(art, provider) {
  try {
    ...
    if (!imageUrl) {
      console.warn(`Skipping artwork ${metadata.title} (${provider}) - no image`);
      return;
    }

    const folder = path.join(OUTPUT_DIR, `${provider}_${safeName(metadata.title)}`);
    await fs.ensureDir(folder);

    await downloadImageWithRetry(imageUrl, path.join(folder, 'image.jpeg'), 3, referer, XSRF_TOKEN);
