    ...
    if (refreshUser && user) {
      refreshUser().then((freshUser) => {
        ...
        if (
          ...
          freshUser.onboardingStatus !== "skipped"
        ) {
        ...
        {proposalMetadata && (
          <div className="bg-white rounded-lg shadow-md p-4">
            ...
            <p className="text-sm text-gray-600">
              Approved: {proposalMetadata.approved}
            </p>
            <p className="text-sm text-gray-600">
              Rejected: {proposalMetadata.rejected}
            </p>
            <p className="text-sm text-gray-600">
              Not Responded: {proposalMetadata.notResponded}
            </p>
            <p className="text-sm text-gray-600">
              Last Updated:{" "}
              {proposalMetadata.lastUpdated
                ? new Date(proposalMetadata.lastUpdated).toLocaleString()
                : "NaN"}
            </p>
          </div>
        )}
      ...
      {(user.sharedCollectionUploads?.length ?? 0) > 0 && (
        <section className="space-y-6">
          ...
          <p className="text-sm text-gray-500">
            ...
            so far. Upload more anytime from the chat or questionnaire.
          </p>
              ...
              user.comments.map((comment, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <div
                        className={`text-xs mb-1 ${isMe ? "text-blue-100" : "text-gray-500"}`}
                      >
                      ...
                      {isImageMessage ? (
                        ...
                        >
                          <div
                            className={`rounded-xl overflow-hidden border ${isMe ? "border-white/30 bg-white/10" : "border-gray-200 bg-gray-50"}`}
                          >
                            <img
                              src={trimmedText}
                          ...
                          <span
                            className={`mt-1 block text-[10px] uppercase tracking-wide ${isMe ? "text-blue-100" : "text-gray-400"}`}
                          >
