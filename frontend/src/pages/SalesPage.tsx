    ...
    function renderQuestionnaire(q: Record<string, unknown>) {
        ...
        return Object.entries(q).map(([k, v]) => {
            ...
            return (
                <div key={k} className="mb-4 border-b border-gray-100 pb-4 last:border-0 last:pb-0">
                    <div className="text-xs font-medium text-gray-500 mb-1">{humanize(k)}</div>
                    <div className="text-sm text-gray-900 font-medium">{formatValue(v)}</div>
                </div>
            );
        });
    }
    ...
    return (
        <div className="p-4 sm:p-6 md:p-8">
            ...
            <div className="mb-8 grid grid-cols-1 md:grid-cols-2 gap-4 max-w-4xl">
                ...
                {isGlobalAdmin && (
                    <div>
                        <label htmlFor="sales-domain" className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">Domain</label>
                ...
                <div>
                    <label htmlFor="sales-user" className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">Customer</label>
                    <div className="relative">
                        <select
                            id="sales-user"
                    ...
                    >
                        {!selectedUserId && (
                            <div className="text-center py-12 bg-gray-50 rounded-2xl border border-dashed border-gray-200">
                                <p className="text-gray-500">Please select a customer above to view their details.</p>
                            </div>
                        )}
                            ...
                            <div className="space-y-8">
                                ...
                                <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 flex flex-col md:flex-row items-start md:items-center gap-6">
                                    <div className="w-20 h-20 rounded-full bg-blue-100 flex items-center justify-center text-3xl font-bold text-blue-600 shrink-0">
                                        {userDetails.name ? userDetails.name.split(' ').map(n => n[0]).slice(0, 2).join('') : (userDetails.email?.[0] ?? 'U').toUpperCase()}
                                    </div>
                                    <div className="flex-1">
                                        <h2 className="text-2xl font-bold text-gray-900">{userDetails.name ?? 'Unnamed User'}</h2>
                                    ...
                                    <div className="px-4 py-2 bg-gray-50 rounded-xl border border-gray-100">
                                        <div className="text-xs text-gray-500 uppercase tracking-wider font-semibold">Onboarding</div>
                                        <div className="text-sm font-medium text-gray-900 mt-1 capitalize">{(userDetails as any).onboardingStatus ?? 'Unknown'}</div>
                                    </div>
                                </div>
                                ...
                                {statsLoading && (
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                        {[...Array(4)].map((_, i) => (
                                            <div key={i} className="bg-gray-50 h-24 rounded-xl border border-gray-100 animate-pulse" />
                                        ))}
                                    </div>
                                )}
                                ...
                                {stats && !statsLoading && (
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                        <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm flex items-center gap-4">
                                            ...
                                            <div>
                                                <div className="text-xs text-gray-500 font-medium uppercase">Total Artworks</div>
                                                <div className="text-xl font-bold text-gray-900">{(stats as any).totalArtworks ?? (stats as any).total ?? '—'}</div>
                                            </div>
                                        </div>
                                        <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm flex items-center gap-4">
                                            ...
                                            <div>
                                                <div className="text-xs text-gray-500 font-medium uppercase">Vectorized</div>
                                                <div className="text-xl font-bold text-gray-900">{(stats as any).vectorized ?? '—'}</div>
                                            </div>
                                        </div>
                                        <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm flex items-center gap-4">
                                            ...
                                            <div>
                                                <div className="text-xs text-gray-500 font-medium uppercase">Indexed</div>
                                                <div className="text-xl font-bold text-gray-900">{(stats as any).indexed ?? '—'}</div>
                                            </div>
                                        </div>
                                        <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm flex items-center gap-4">
                                            ...
                                            <div>
                                                <div className="text-xs text-gray-500 font-medium uppercase">Feedback</div>
                                                <div className="text-xl font-bold text-gray-900">{(stats as any).withFeedback ?? '—'}</div>
                                            </div>
                                        </div>
                                    </div>
                                )}
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                    ...
                                    <div className="lg:col-span-2 bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                                        <div className="flex items-center gap-2 mb-6 pb-4 border-b border-gray-100">
                                            ...
                                            <h3 className="text-lg font-bold text-gray-900">Onboarding Questionnaire</h3>
                                        </div>
                                    ...
                                    <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 h-fit">
                                        <h3 className="text-sm font-bold text-gray-900 uppercase tracking-wider mb-4">System Data</h3>
                                                ...
                                                if (isPrimitive) {
                                                    return (
                                                        <div key={k} className="flex justify-between items-center py-2 border-b border-gray-50 last:border-0">
                                                            <span className="text-xs text-gray-500 font-medium">{humanize(k)}</span>
