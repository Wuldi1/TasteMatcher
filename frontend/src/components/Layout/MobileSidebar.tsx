...
export const MobileSidebar = () => {
  ...
  const { currentStep, nextStep, skipTour, isTourActive, previousStep } =
    useWelcomeTour();
        ...
        {filteredLinks.map((link) => {
          ...
          return (
            <div key={link.id} className="relative group flex-1">
              ...
              {/* Welcome Tour Bubble */}
              {isActiveBubble && (
                <div className="absolute bottom-20 left-1/2 transform -translate-x-1/2 z-50 bg-gradient-to-r from-purple-500 to-purple-600 text-white shadow-lg rounded-lg p-4 w-24 text-center">
                  {/* X Button */}
                  <button
                    onClick={skipTour}
