    ...
    >
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col gap-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <div
            className={`px-4 py-2 rounded-full text-sm font-semibold flex items-center gap-2 capitalize ${
        ...
        {generalComments.length > 0 && (
          <div className="space-y-3 mb-6">
            {generalComments.map((comment, index) => (
              <div
                key={index}
                className={`p-3 rounded-lg border ${comment.author === "Customer" ? "bg-blue-50 border-blue-100 ml-8" : "bg-gray-50 border-gray-100 mr-8"}`}
              >
                ...
                <div className="lg:w-1/3 xl:w-1/4 bg-gray-50 relative group">
                  {artwork?.filename ? (
                    <div className="w-full lg:h-full relative bg-gray-100 flex items-center justify-center overflow-hidden">
                      ...
                      >
                        <span className="bg-white/90 text-gray-900 text-xs font-medium px-3 py-1.5 rounded-full shadow-sm">
                          View Full Size
                        </span>
                      </a>
                    </div>
                ...
                <div className="flex-1 p-6 flex flex-col">
                  <div className="flex justify-between items-start gap-4 mb-4">
                    ...
                    <div className="flex flex-col items-end gap-2">
                      ...
                      <span
                        className={`inline-flex items-center gap-1.5 px-3 py-1 text-xs font-semibold rounded-full ${color}`}
                      >
                        {icon}
                        {text}
                      </span>
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm text-gray-600 mb-6">
                    ...
                    <div>
                      <span className="block text-xs text-gray-400 uppercase tracking-wider">
                        {artwork?.isAuction ? "Price Range" : "Price"}
                      </span>
                      {priceDisplay}
                  ...
                  <div className="mt-auto pt-6 border-t border-gray-100 flex gap-3">
                    <button
                      onClick={() =>
                        handleArtworkStatusChange(item.artworkId, "approved")
                      }
                      disabled={isReadOnly}
                    ...
                    <button
                      onClick={() =>
                        handleArtworkStatusChange(item.artworkId, "rejected")
                      }
                      disabled={isReadOnly}
                      className={`flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg font-medium transition-colors ${
                  ...
                  <div className="flex-1 space-y-3 overflow-y-auto max-h-60 lg:max-h-none mb-3 pr-1 custom-scrollbar">
                    {item.comments.length === 0 ? (
                      ...
                      item.comments.map((comment: Comment, index: number) => (
                        <div
                          key={index}
                          className={`p-3 rounded-lg text-sm ${comment.author === "Customer" ? "bg-blue-50 border border-blue-100 ml-4" : "bg-white border border-gray-200 mr-4"}`}
                        >
                          <div className="flex justify-between items-baseline mb-1">
