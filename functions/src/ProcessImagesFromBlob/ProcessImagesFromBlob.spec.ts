...
describe("ProcessImagesFromBlob", () => {
  ...
  beforeEach(() => {
    ...
    mockThumbnailService =
      new ThumbnailService() as jest.Mocked<ThumbnailService>;
    mockVectorizationService =
      new VectorizationService() as jest.Mocked<VectorizationService>;
    mockSearchIndexService =
      new SearchIndexService() as jest.Mocked<SearchIndexService>;
  });
  ...
  it("should process valid message with thumbnails and vectorization", async () => {
    ...
    ).toHaveBeenCalledWith(mockImageBuffer, "artwork-456");
  ...
  it("should handle thumbnail generation failure", async () => {
    ...
    mockThumbnailService.generateAndUploadThumbnails.mockRejectedValue(
